...
public class FileAppLayer implements BaseLayer {
    ...
    public boolean sortFileList(int lastFrameNumber) {
        ...
        if((fileByteList.size() - 1 != lastFrameNumber) || (receivedLength != targetLength)) {
            ((ApplicationLayer)this.GetUpperLayer(0)).ChattingArea.append("파일 수신 실패\n"); // 수신 실패 메시지
            return false;
        }
        /*
         * 모든 프레임을 받은 경우
    ...
    public void setAndStartSendFile() {
        ApplicationLayer upperLayer = (ApplicationLayer) this.GetUpperLayer(0);
        ...
        try (FileInputStream fileInputStream = new FileInputStream(sendFile)) {  // 파일스트림을 가져 옴
            ...
            ((ApplicationLayer)this.GetUpperLayer(0)).progressBar.setMaximum(sendTotalLength);
            ...
            if(sendTotalLength <= 1448) {
                ...
                ((ApplicationLayer)this.GetUpperLayer(0)).progressBar.setValue(sendedLength); // progressbar 현재 진행값 업데이트
            }
    ...
    public synchronized boolean Receive(byte[] input) {
        ...
        if(checkReceiveFileInfo(input)) {
            ...
            ((ApplicationLayer)this.GetUpperLayer(0)).progressBar.setMinimum(0);
            ((ApplicationLayer)this.GetUpperLayer(0)).progressBar.setMaximum(targetLength);
            ((ApplicationLayer)this.GetUpperLayer(0)).progressBar.setValue(0);
            ...
            /* 단편화를 진행한 데이터를 받은 경우 */
                ...
                if(checkLastDataFrame(input)) {
                    ...
                    if(sortFileList(lastFrameNumber)) {
                        try(FileOutputStream fileOutputStream = new FileOutputStream(file)) {
                            ...
                            ((ApplicationLayer)this.GetUpperLayer(0)).ChattingArea.append("파일 수신 및 생성 완료\n");  // 수신 완료 메시지
                            fileByteList = new ArrayList();
                ...
                ((ApplicationLayer)this.GetUpperLayer(0)).progressBar.setValue(receivedLength);  // Progressbar 갱신
            }
        }

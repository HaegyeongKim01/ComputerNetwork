...
public class ARPLayer implements BaseLayer {
	...
	/* ARP 캐시 테이블에 해당 IP가 있는지 확인한다 */
	public int hasIpInCacheTable(byte[] src_ip, byte[] dst_ip) {
		// 없으면 기본값 -1 반환
		int idx = -1;
	    for(int i=0; i<cacheTable.size(); i++) {
	    	//만약 목적지 IP 주소가 캐시 테이블에 있다면 해당 인덱스를 반환
	    	if(java.util.Arrays.equals(dst_ip, cacheTable.get(i).get(0))) { 
	    		idx = i;
	        }
	    }
	    return idx;
	...
	/* 캐시 테이블에 해당 인덱스에 위치한 Mac 주소를 읽어온다. */
